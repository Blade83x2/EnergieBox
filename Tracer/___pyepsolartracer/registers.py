...
def A():
    return [ 'Ampere', 'A' ]
def AH():
    return [ 'Amperestunden', 'Ah' ]
def W():
    return [ 'Watt', 'W' ]
def C():
    return [ 'Grad Celsius', ' Grad Celsius' ]
def PC():
    return [ '%, Prozent', '%' ]
def KWH():
    return [ 'kWh, kiloWatt/Stunde', 'kWh' ]
def Ton():
    return [ '1000kg', 't' ]
def MO():
