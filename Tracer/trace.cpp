    ...
    void speichereInDatenbank(float pv_volt, float pv_ampere, float pv_power, float batt_volt, float batt_ampere, float batt_power, int batt_soc, float generated_power, int grid_load_active) {
        ...
        if (!mysql_options(conn, MYSQL_READ_DEFAULT_FILE, "/home/box/.mysql_energiebox.cfg")) {
            if (mysql_real_connect(conn, nullptr, nullptr, nullptr, nullptr, 0, nullptr, 0)) {
                ...
                std::string sql = "INSERT INTO messwerte (timestamp, pv_volt, pv_ampere, pv_power, batt_volt, batt_ampere, batt_power, batt_soc, generated_power, grid_load_active) VALUES (" +
                    std::to_string(timestamp) + ", " +
                    std::to_string(pv_volt) + ", " +
                    std::to_string(pv_ampere) + ", " +
                    std::to_string(pv_power) + ", " +
                    std::to_string(batt_volt) + ", " +
                    std::to_string(batt_ampere) + ", " +
                    std::to_string(batt_power) + ", " +
                    std::to_string(batt_soc) + ", " +
                    std::to_string(generated_power) + ", " +
