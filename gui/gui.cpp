...
class GUI : public Gtk::Window
{
  ...
  private:
		...
		if ( tabName.find( "Energiebox" ) != std::string::npos )
		{
			...
			if ( !energiebox_timer_connection_.connected() )
			{
				energiebox_timer_connection_ = Glib::signal_timeout().connect_seconds(
					sigc::mem_fun( *this, &GUI::update_energiebox_tab ), 61 );
	...
	bool check_idle_time()
	{
		...
		{
			if ( notebook_.get_current_page() != 0 )
			{  // Wenn nicht schon Info-Tab
	...
	Gtk::Grid *create_data_table( const std::vector<std::pair<std::string, std::string>> &data,
